...
namespace ToonKids
{
    public class blendshapesshow : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            for (int forAUX = 0; forAUX < Heads.Length; forAUX++)
            {
                if (forAUX < (Heads.Length / 2)) GOAUX[forAUX].transform.position = transform.position + (transform.right * (forAUX - 2f) * -0.25f);
                if (forAUX > (Heads.Length / 2) - 1) GOAUX[forAUX].transform.position = transform.position + (transform.right * (forAUX - 7f) * -0.25f) + (transform.up * -0.5f);
            }
        }
