...
namespace ToonKids
{
    public class TK3DController : MonoBehaviour
    {
        ...
        public RuntimeAnimatorController ControllerAnimations;
        ...
        private void Start()
        {
            ...
            anim.runtimeAnimatorController = ControllerAnimations;
            ...
            Head = Model.transform.Find("ROOT/TK/TK Pelvis/TK Spine/TK Spine1/TK Spine2/TK Neck/TK Head");
            Glasses = Model.transform.Find("ROOT/TK/TK Pelvis/TK Spine/TK Spine1/TK Spine2/TK Neck/TK Head/Glasses");
            GlassesONOFF = Glasses.gameObject.activeSelf;
            Pelvis = Model.transform.Find("ROOT/TK/TK Pelvis");
        ...
        void SetMoveDir3()
        {
            if (Physics.Raycast(trans.position + Vector3.up * tall * 0.5f + dirforw * 0.0125f, -Vector3.up, out RaycastHit hit, tall * 0.6f))
            {
                if (Vector3.Angle(hit.normal, Vector3.up) > 0.5f)
                    dirforw = Vector3.Slerp(dirforw, Vector3.Cross(Vector3.Cross(Vector3.up, hit.normal), hit.normal), 25f * Time.deltaTime).normalized;
                else
        ...
        void SelectChar()
        {
            ...
            if (!Boy && !Girl) randgender = true;
            if (Boy && Girl) randgender = true;
            if (Boy && !Girl) Character = Instantiate(Boys[Random.Range(0, 5)], transform);
            if (Girl && !Boy) Character = Instantiate(Girls[Random.Range(0, 5)], transform);
            if (randgender)
            {
                int AUX = Random.Range(0, 10);
                if (AUX < 5) Character = Instantiate(Boys[AUX], transform);
                else Character = Instantiate(Girls[AUX - 5], transform);
            }
